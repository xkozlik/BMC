...
The tunnels are detected on selected protein conformations, resulting, e.g., from MD simulations or from a tool providing the protein conformation sampling
...
\subsection*{Traversability of Tunnels}
...
Protein tunnel is described by a sequence of collision-free spheres $T=( (c_1, r_1),\ldots,(c_n,r_n) )$, where $n$ denotes the number of spheres, $c_i \in$ $\mathbb{R}^3$ their 3D position, and $r_i > 0$ the maximum collision-free radius of a sphere centered at $c_i$. 
...
\subsubsection*{Computing Initial Configurations}
...
\subsubsection*{RRT for Computing Ligand's Trajectory along Tunnel}
