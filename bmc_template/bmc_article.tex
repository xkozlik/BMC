...
The method~\cite{cortes2010simulating} computes any exit pathway without allowing the user to focus the search to a selected tunnel only.
...
In contrast to the method proposed by Cort{\'e}s et al.~\cite{cortes2010simulating}, which searches for any exit pathway from the protein, we localize the searching into detected tunnels and compute the trajectories only along each tunnel separately.
...
These atom positions are used to determina the distance between two configururations $q,q' \in \C$ of the ligand:

\begin{equation}
\dist(q,q') = \sum_{i=0}^{n_l} d( a_i(q),a_i(q') )
\end{equation}

\noindent
where $n_l$ is the number of atoms of the ligand, and $d()$ is the 3D Euclidean metric.
...
The trajectories are found using the RRT principle~\cite{lavalleRRT} that is extended to focus the search towards the selected protein tunnel.
...
The default settings of CAVER 3.0 were used.
The traversability was then computed for the first two tunnels (the tunnels were ranked using the CAVER 3.0).
...
The ligands:
DCP (12 atoms, 2 DOFS)
m003,m037t,207 (4 atoms, 1 dof)
m038t (5 atoms, 2 dofs)
m062r,m113,m210e (5 atoms, 1 dof)
m080,TBP (6 atoms, 2 dofs) 
m112 (6 toms, 3 dofs)
m133 (4 atoms, 0 dofs)
m141 (6 atoms, 2 dofs)
