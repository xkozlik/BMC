...
%Computationally less demanding approaches than MD for generating possible 
%Protein conformations can be also explored using constrained geometric simulations, such as
ROCK~\cite{lei2004sampling}, FRODA~\cite{wells2005constrained}, and tCONCOORD~\cite{seeliger2007geometry}.
Although these approaches are faster than classic MD, they are still too slow for processin large datasets.
...
To attract the tree towards a difficult region, random samples have to be generated there only if the tree can expand to this region.
...
The guiding path can be computed in the workspace~\cite{vonasek2009rrt}, 
iteratively refined in the configuration space based on the solution of a relaxed version of the problem~\cite{bayazitIRC}, or
even specified by a human operator~\cite{denny2018general}.

The advantange of sampling-based planners is the ability to cope with high-dimensional systems.
The flexibility of ligands (and proteins) is achieved by modelling them as a kinematic chain~\cite{cortes2010simulating}.
This makes the sampling-based planners suitable for various biochemistry tasks, 
%Besides many applications in robotics, the sampling-based planners have been already used in biochemistry as well, 
e.g., to study loop motions~\cite{cortes2004geometric}, 
to identify binding sites~\cite{bayazit2001ligand,singhLIG},
for exit pathway detection~\cite{cortes2010simulating,cortes2005path}, 
for tunnel detection~\cite{vonasek2017tunnel}, 
and for protein folding~\cite{raveh2009rapid,amato2002using}. %novinskaya2015improving
...
The computational burden of the sampling-based planners strongly depends on the number of DOFs to be considered.
Therefore, several methods attempt to reduce the DOFs to speed up the search.
In ART-RRT, a combination of the RRT planner and As-Rigid-As-Possible (ARAP) morphing technique from Computer Graphics 
is used to decrease the number of DOFs.
Similarly, in the tool MoMa-LigPath~\cite{cortes2010simulating}, only a subset of DOFs are used for the planning by 
using RRT-ML planner.
%The most relevant methods to our research is the MoMa-LigPath tool~\cite{cortes2010simulating} that computes exit pathways
...
%ART-RRT \cite{nguyen18art}

%from \cite{nguyen18art}:
%ARAP technique is used as a morphing and dimensionreduction method for the ligand, while an energy minimization
%is used to passively adapt the protein motions according to the ligand motions. As a result, the ART-RRT method efficiently
%produces low-energy pathways, which can then be refined further with path-optimization methods such as the Nudged Elastic Band[46] or String[47] methods for finding minimum-energy paths (MEP). 
%The paths produced by the ART-RRT method can also be used to start the Transition Path Sampling for computing free-energy differences[48] or reaction rate constants.[49]
...
$iteration = 0$ \tcp*{\ccolor iteration counter}
...
\While{$iteration < \Imax$ {\bf and}  $v < n$}{
    ...
    $\T=$expand($\T, \qnear,\qrand$) \tcp*{\ccolor Alg.~\ref{alg::expand}}
    \For(\tcp*[f]{\ccolor find the last reached sphere}){$i=n,n-2,\ldots,v$}{ \nllabel{alg::main:a} 
...
\subsection*{Traversability of single protein conformations}

The performance of the proposed method was verified on a set of various proteins and ligands and compared 
to the MoMa-LigPath (MMLP) tool~\cite{cortes2010simulating}.
MMLP was run in the default settings including the scale factor 0.7.
The same scale factor was used also in our method.
Other settings of out method are: $\Imax=100 \cdot 10^3$.

In each protein, tunnels for spherical probe 1.0~\AA\ were found using CAVER 3.0~\cite{caver3} (using default settings).
The found tunnels were ranked using the default CAVER 3.0 system.
The first two tunnels were then used in the experiments.

The comparison was made using the following nine proteins:
1AKD (3208 atoms),
1BL8 (2823 atoms),
1MXT (8343 atoms),
1PV7 (6581 atoms),
...
\begin{figure}
\begin{tabular}{cc}
\includegraphics[width=0.7\textwidth]{fig/tunnel_1} &
%\includegraphics[width=0.5\textwidth]{fig/tunnel_2} \\
